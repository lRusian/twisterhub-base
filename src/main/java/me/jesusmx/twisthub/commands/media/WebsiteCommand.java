...
public class WebsiteCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (ConfigFile.getConfig().getBoolean("commands.website.enabled"))
        ConfigFile.getConfig().getStringList("commands.website.lines").stream().map(CC::translate).forEach(sender::sendMessage);
        return true;
    }
}
