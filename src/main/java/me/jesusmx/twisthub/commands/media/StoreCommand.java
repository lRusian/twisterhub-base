...
public class StoreCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (ConfigFile.getConfig().getBoolean("commands.store.enabled"))
            ConfigFile.getConfig().getStringList("commands.store.lines").stream().map(CC::translate).forEach(sender::sendMessage);
        return true;
    }
}
